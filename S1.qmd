...
### check the difference between animal cognition and life science ----
## glm with poisson distribution
# change the date format 
dat_fig1_long <- dat_fig1 %>%
  pivot_longer(cols = -year, names_to = "group", values_to = "pub")

# compare the increase rate in research paper
df_research <- dat_fig1_long %>%
  filter(group %in% c("animcog_research", "biol_research"))

model_research <- glm(pub ~ year * group, data = df_research, family = poisson(link = "log"))
summary(model_research)

# compare the increase rate in reviews 
df_review <- dat_fig1_long %>%
  filter(group %in% c("animcog_review", "biol_review"))

model_review <- glm(pub ~ year * group, data = df_review, family = poisson(link = "log"))
summary(model_review)

## lm with log transformed
model_research_lm <- lm(log(pub) ~ year * group, data = df_research)
summary(model_research_lm)


model_review_lm <- lm(log(pub) ~ year * group, data = df_review)
...
fig_4a <- ggplot(dat_species, aes(x = research_aim, y = species_count, fill = research_aim)) +
...
dat_tax_long <- dat_fig4 %>%
  filter(!is.na(taxon)) %>%
  separate_rows(taxon, sep = ";") %>%
  mutate(taxon = str_trim(taxon),
         research_aim = str_to_title(research_aim))  # Capitalise labels
...
fig_4b <- ggplot(dat_tax_counts, aes(x = n, y = factor(taxon, levels = taxon_order), fill = research_aim)) +
...
# Plot
fig_5a <- ggplot(dat_fig5a_long, aes(x = fct_rev(item), y = percent, fill = value)) +
  geom_col(width = 0.8) +
  coord_flip() +
...
# Plot
fig_5b <- ggplot(dat_fig_5b_long, aes(x = fct_rev(item), y = percent, fill = factor(value, levels = levels_4))) +
  geom_col(width = 0.8) +
  coord_flip() +
...
```{r}
## do not run on the qmd, but do run on console
# Set custom colors 
cols <- c(
  USA = "#00688B",
  China = "#8B6508",
...
file <- "data/bibliometric/scopus.bib"
bib_data <- convert2df(file = file, dbsource = "scopus", format = "csv")

bib_data <- convert2df(here("data","bibliometric", "scopus.bib"), dbsource = "scopus", format = "bibtex")
dim(bib_data) 
...
# primary studies ---
# Load and process
bib_data_primary <- convert2df(file = here::here("data", "bibliometric", "scopus_primary.csv"), 
           dbsource = "scopus", 
           format = "csv")
